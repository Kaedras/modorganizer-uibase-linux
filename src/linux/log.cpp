...
protected:
  void sink_it_(const spdlog::details::log_msg& m) override
  {
    ...
    try {
      ...
      fprintf(stderr, "uncaught exception in logging callback, %s\n", e.what());
    } catch (...) {
...
void Logger::setFile(const File& f)
{
  ...
  if (m_file) {
    auto* ds = dynamic_cast<spdlog::sinks::dist_sink<std::mutex>*>(m_sinks.get());
...
{
  if (m_callback) {
    dynamic_cast<CallbackSink*>(m_callback.get())->setCallback(f);
  } else {
...
{
  if (filter.empty() || replacement.empty()) {
    // nothing to do
    return;
  }
